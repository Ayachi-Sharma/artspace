...
// the workshop's earliest sessionDate (a booking covers the whole workshop,
// not a single session, so the first date is what counts as "start").
// Adjust the constant below to change the window.
...
export async function POST(
  ...
  try {
    ...
    if (booking.status === "failed") {
      return NextResponse.json({ error: "This booking never succeeded — nothing to cancel" }, { status: 409 });
    }
    ...
    if (workshop.sessionDates?.length) {
      const earliestStart = new Date(
        Math.min(...workshop.sessionDates.map((d: Date) => new Date(d).getTime()))
      );
      const hoursUntilStart = (earliestStart.getTime() - Date.now()) / (1000 * 60 * 60);
