...
export async function POST(req) {
  try {
    ...
    // Verify the HMAC signature Razorpay sends back to the client.
    ...
    if (booking.status === "failed") {
      return NextResponse.json(
        {
          error:
            "This booking already failed — refund was already initiated",
          bookingId: booking._id,
    ...
    if (!workshop) {
      // Payment succeeded but the workshop sold out in the meantime.
      // Refund immediately.
      ...
      return NextResponse.json(
        {
          error:
            "Workshop sold out after payment — refund initiated",
          bookingId: booking._id,
          refundStatus: booking.refundStatus,
        },
        { status: 409 }
