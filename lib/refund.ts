...
export async function attemptRefundWithRetry(params: {
  ...
  amount: number; // paise
  reason: string; // e.g. "cancellation", "sold_out_race"
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      lastError = err?.error?.description || err?.message || "Unknown Razorpay error";
      console.error(`Refund attempt ${attempt}/${MAX_ATTEMPTS} failed for payment ${razorpayPaymentId}:`, lastError);
  ...
  try {
    ...
    console.error("CRITICAL: could not persist FailedRefund record", dbErr);
  }
